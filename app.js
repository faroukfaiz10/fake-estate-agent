...
const fetchData = async () => {
    const URL = "https://trouverunlogement.lescrous.fr/tools/flow/21/search?bounds=2.1601_48.9661_2.5391_48.6926"
    ...
    setInterval(
        async () => {
            ...
            const residences = await asyncFilter(results, async result => {
            ...
            if (residences.length){
                const residencesListMessage = residences.map(residence => `  - ${residence.name}`).join("\n")
                notifyDiscord(`Hey, I found ${residences.length} available residences!\n${residencesListMessage}`)
            }
        }, 1000 * 60 * 5
    )
}
...
const notifyDiscord = async (message) => {
    const DISCORD_URL = 'https://discord.com/api/webhooks/936304587244204073/MDHxQYyzB5UqsFYkk8AfEfSFhXc19oXTk1BaN7LKLv7_ZZl8BDGgWousKN_SUjaXgRx3';
    var payload = JSON.stringify({
        username: "Fake estate agent",
        avatar_url: "https://p7.hiclipart.com/preview/866/59/617/estate-agent-real-estate-computer-icons-house-property-house.jpg",
        content: message
    });
    var params = {
        headers: {
